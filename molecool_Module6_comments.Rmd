...
```
 ...
 <div class="comment">
Haven't saved your summarized data here, only produced a plot. You need the tempeature difference as the prompt asked for.
</div>
...
```
 ...
 <div class="comment">
These models don't include the temperature difference. 
</div>
...
```
 ...
 <div class="comment">
Great work on producing nice tables.

But here's what needed to happen both in terms of modeling and preparing the human data so that it could be added to the other mammal data.
...
##linear model
  dat.log.lm <- lm(Td~mass,dat)
  dat.log.allo <- nls(Td~a*mass^b, start=list(b=1, a=1),data = dat)
...
#Prepare for merging with mammal data. Notice the log transformation

human.dat <- dat%>%
  ungroup()%>%
  summarize(temp=mean(log(Td),na.rm = TRUE),mass.g=mean(log(mass*1000),na.rm = TRUE),species="Homo sapians",Order="Primata")%>%
  dplyr::select(Order,species,mass.g,temp)
...
```
 ...
 <div class="comment">
Please don't print big data tables. Also, you didn't add the human data . . .what the prompts asked for. Here's how.
...
#now the modeling which include humans
mammal.lm <- lm(temp~mass.g,mammal.log)

mammal.allo <- nls(temp~a*mass.g^b, start=list(b=0.1, a=0.1),data = mammal.log)

#AIC and a nice table (which wasn't included)
...
```
 ...
 <div class="comment">
Why both and AIC table and AICw table? The latter includes all the info in the former.
</div>
...
```
 ...
 <div class="comment">
We only need one table for the AICw. And we don't need to run anovas on models that don't fit well (the one without order)

Also, it's easy to print nicer looking anova tables with `broom`

```{r}
require(broom)#install if not

anova(mammal.log.Order.lm)%>%tidy()%>%kable(capiton="some informative caption")
```

I also think the regression of the orders does a better job at representing the different relationships between orders than the residuals plot below.
</div>
