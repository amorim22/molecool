...
pseed.sum.max <- pseed.wide%>%
  group_by(date)%>%
  mutate(peak=frame %in% find.peaks(frame,amp.sum))%>%
  filter(peak==T) #all above was finding mean
...
amp.sum <- pseed.sum.max%>% pull(amp.sum)  #this is defining the amp.sum.mean column in the tibble as a vector

standard_error <- function(amp.sum) sd(amp.sum) / sqrt(length(amp.sum)) #defines standard error function

pseed.sum.max<- pseed.sum.max %>% #makes standard error column                                  
  group_by(fish,bl.s)%>%  
  mutate(amp.sum.se = standard_error(amp.sum))
pseed.sum.max  #delete later
