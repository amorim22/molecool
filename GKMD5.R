...
for(i in dat.f){
  ...
  dat.l[[i]]<- read_csv(i)%>%
    mutate(who="Steph", who="Jess", who="Julia", who="Greg", activity="active", activity="Couch") #read, then store data with additional columns of metada in list as position unique to i
