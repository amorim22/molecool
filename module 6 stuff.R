...
mammal <- mammal %>% mutate(temp.species=T.high - T.low)

print(mammal)

mammal.log <- mammal%>%
  mutate_at(c("temp.species", "mass.g"),log)

mammal.lm <- lm(temp.species~mass.g,mammal)

mammal.allo <- nls(temp.species~a*mass.g^b, start=list(b=0.1, a=0.1),data = mammal)
...
mammal.log%>%
  ggplot(aes(mass.g,temp.species,col=Order))+geom_point()+geom_smooth(method="lm")

mammal.log.Order.lm <- lm(temp.species~mass.g*Order,mammal.log)
summary(mammal.log.Order.lm)
anova(mammal.log.Order.lm)

mammal.log.lm  <- lm(temp.species~mass.g,mammal.log)
anova(mammal.log.lm)
